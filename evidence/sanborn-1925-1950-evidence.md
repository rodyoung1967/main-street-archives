# Sanborn Evidence Note — 1925 / 1950 Main Street Comparison

Evidence ID: `EV-SAN-1925-1950-001`
Related register entry: `E-020`
Evidence type: Primary map evidence
Sources: `S-021`, `S-022`
Maps: `SM-008`, `SM-009`
Related file: `maps/sanborn-comparison-503-505-507.md`
Confidence: High for map-visible address and occupancy pattern; exact abbreviation readings still need high-resolution confirmation.

## Core finding

The 1925 and 1950 Sanborn sheets support treating 503 Main Street, 505 Main Street, and 507 Main Street as separate mapped address and occupancy records.

The 1925 sheet also directly addresses the earlier 501 / 503 / 505 question because it shows separate Main Street labels including `501`, `503`, `505`, and `507` near 5th.

## 1925 working reading

- `501` appears as a separate store space near the 5th/Main corner.
- `503` is labeled restaurant, with a concrete-floor note visible.
- `505` is labeled billiards and cigars.
- `507` appears as a separate commercial space; the exact wording still needs careful confirmation.

## 1950 working reading

- `503` remains a separate restaurant-labeled occupancy.
- `505` is labeled with billiards, cigars, and beer-related use; restaurant wording is also visible nearby.
- `507` appears as a separate store space.

## Archive implications

- Keep 503 Main and 505 Main as separate address/building records.
- Treat the 1925 map as a partial answer to the 501 Main question.
- Treat the 503 restaurant label as consistent with later Kwality Cafe research, but not proof of the Kwality name.
- Treat the 505 billiards/cigars/beer pattern as useful background for later 505 tavern/bar-lineage research.
- Keep the reported 1919 construction date as a working hypothesis rather than proof of the first commercial frontage at this location.
