# 1925–1926 503 / 505 Main business-identification checkpoint

Status: **research checkpoint; 1925 trade names/operators remain unresolved**

Review date: **1 September 2026**

Purpose: preserve the completed portion of the focused pass to identify the restaurant mapped at **503 Main** and the billiards/cigars business mapped at **505 Main** on the May 1925 Sanborn. This note also records incidental Fifth Street evidence without converting intersection-level references into numbered Main Street addresses.

## Firm source boundary

The visually reviewed **1925 Sanborn Sheet 6** (`E-020` / `S-021` / `SM-008`) proves the following uses and nothing more:

- **503 Main — restaurant**
- **505 Main — billiards/cigars**

Sanborn does **not** print either trade name or proprietor. A compatible earlier or later business cannot be assigned to the 1925 premises solely from its type of trade.

## 503 Main result

Oregon City's official historic-resource synthesis (`S-026`) says that:

- the 1925 Sanborn depicts a restaurant at 503; and
- **1926 newspaper advertisements** identify the business as **Kwality Restaurant**.

The underlying 1926 advertisement, page, date, and proprietor wording were **not recovered** during this pass. The local newspaper calendars checked online do not expose a usable 1925 or 1926 run for the *Banner-Courier* or *Oregon City Enterprise*. Therefore:

- **Kwality Restaurant at 503 in 1926** remains an official secondary-source synthesis pending recovery of its cited advertisement;
- **Kwality is a strong next-year candidate for the unnamed 1925 restaurant**, but is **not yet proved to be the May 1925 map occupant**;
- the undated New Kwality Cafe and Larry's Kwality Cafe artifacts remain later/undated exact-address anchors and do not fill the 1925 gap.

The **Quality Restaurant at 427 Main** advertised on 8 June 1922 is a different exact-address record and must not be merged into the 503 Kwality sequence without transfer evidence.

## 505 Main result

The official Farr residence history (`S-036` / `E-033`) proves that Edwin F. and Alice Farr were proprietors of **Farr's Pool Hall at 505 Main**, but it does not supply the pool hall's opening, closing, or acquisition date. No address-specific source recovered in this pass places Farr's at 505 in May 1925. Therefore:

- **Farr's is confirmed at 505 at an unresolved date**;
- it is a plausible candidate for the unnamed 1925 billiards/cigars premises, but the archive must **not identify the 1925 map occupant as Farr's yet**;
- A. Leland's visually verified 1920 permission to operate a pool hall at **505½ Main** (`E-135` / `S-150`) is a separate earlier anchor, not proof of continuity to 1925;
- the 1922 Timms / Leland & Little billiards and cigar evidence is functionally compatible but lacks an exact 505 address bridge;
- Harr's Recreation Center is independently documented later and remains a separate business from Farr's.

## 1925–1926 online newspaper coverage

The public Historic Oregon Newspapers calendar views checked for the following local titles did not expose digitized 1925 or 1926 issues:

- *Banner-Courier* — LCCN `sn00063699`
- *Oregon City Enterprise* — LCCN `sn00063700`

This is a statement about the checked online interface, **not evidence that the newspapers did not publish or that no relevant article exists**. The city synthesis's claimed 1926 Kwality advertisements should be pursued on Oregon City Public Library microfilm or in the municipal historic-resource research file.

## 1923–1924 regional newspaper bridge search

A follow-up online pass inventoried **1,487 issues / 12,096 OCR pages** across seven exposed regional titles for 1923–1924. OCR was used only to identify candidate scans. Exact-name-like and strongest Oregon City-context candidates were then visually checked.

The scan review found only unrelated Farr personal/news references, an ordinary “quality” OCR error, `Jager` errors produced by **Manager Harris** and **Hager**, and Fifth Street references in Salem or Estacada. It produced no address-specific bridge for Farr's at 505, Kwality at 503, or another Oregon City Fifth Street premise. One failed OCR endpoint was resolved by direct inspection of the available page PDF and is not a missing-scan gap.

This narrows the online lead set but does **not** identify either May 1925 occupant and does **not** establish business absence. See:

- `1923-1924-regional-business-ocr-discovery.md`
- `1923-1924-regional-business-candidate-review-2026-09-01.md`
- `1923-1924-regional-business-ocr-discovery.json`

## Municipal Activities Reports checked

Official city source page:
https://www.orcity.org/1710/Historic-Documents-and-Research

- **1925 Municipal Activities Report:** https://www.orcity.org/DocumentCenter/View/6214/1925-Municipal-Activities-Report
- **1926 Municipal Activities Report:** https://www.orcity.org/DocumentCenter/View/6215/1926-Municipal-Activities-Report

**1 September visual correction:** the original report's printed p. 13 (PDF page 7) gives **April 30, 6:15 a.m., Main between Fifth and Sixth, spark from chimney, approximately $15 damage** (`E-139` / `S-157`). The earlier April 18 / $2,500 lead conflated neighboring rows and is withdrawn; its old filename is retained with a supersession notice. No street side, numbered building or occupant is printed. The [full report review](1925-municipal-report-fire-correction-and-fifth-street-review-2026-09-01.md) also preserves an unnumbered August 18 Fifth Street flue call and newly specific receipt/inspection/engineering-record leads. The original 22-page PDF is in the repo, and the [subsequent continuation](1925-municipal-report-remaining-pages-review-2026-09-01.md) completes all 22 pages. Aggregate franchise/license revenue and police-recordkeeping context (`E-141`) supply no named occupant; underlying receipts and restaurant-inspection returns remain retrieval targets. This report's completion is not annual completion.

The 1926 report's Fire Department list was visually inspected on the relevant pages. It includes:

- **29 May, 6:30 p.m. — 8th and Main**, unknown cause, approximately $500 damage. OCR had misread the location as **5th**; the page image corrects it to **8th**. Do not record this as a Fifth/Main incident.
- **4 August, 2:00 a.m. — Paramount Restaurant**, undetermined cause, approximately $4,500 damage. The table gives no street address. Do not assign Paramount Restaurant to 503 Main without an independent address source.
- **11 November, 1:15 p.m. — 5th and Main, false alarm**. This is intersection-level context only; it does not identify 501, 503, 505, a Fifth Street number, or a business.

The reports provide aggregate restaurant inspection/license information but did not yield a named 503 or 505 occupant in this pass.

## CCHS public catalog coverage

The Clackamas County Historical Society public CatalogIt interface was searched for **Farr**, **Kwality**, **505 Main**, **503 Main**, and **Quality Cafe**. No new 1920s target-business storefront image was surfaced. The 505 query returned the already known George Harding Drug Store / George Young Second Hand Store material.

This is a bounded catalog search result, not proof that no relevant image exists in the collection.

## Incidental Fifth Street handling

The research instruction is to preserve other Fifth Street addresses encountered during this work. Apply the following rule:

- record an exact Fifth Street number when the source prints one;
- preserve **Fifth & Main** or **5th and Main** as intersection-level context when that is all the source says;
- do not translate an intersection reference into 501, 503, 505, or another numbered Main Street storefront without independent address-mapping evidence.

The only new Fifth-related item from this municipal-report pass is the **11 November 1926 false alarm at 5th and Main**. It identifies no building or business.

## Highest-value next retrievals

**Municipal continuation, 1 September 2026:** The new Laserfiche catalog is reachable by folder browsing and Advanced Name search. Saved an 81-entry / 159-listed-page 1925 inventory, not completed scan coverage. Six separately retrieved May 1927 pages were visually reviewed without identifying the target occupants; Commissioner Farr is not linked to the pool hall. The export test hit a browser URL-policy restriction and was stopped. See [working route and precise next batches](1924-1927-municipal-minutes-access-review-2026-09-01.md). The two occupant questions below remain open.

**1924 directory follow-up, 1 September 2026:** The NPS bibliography citation was visually reverified and its original PDF preserved, but no readable 1924 directory or confirmed current holding was located. No occupant claim changed. See `1924-oregon-city-directory-retrieval-review-2026-09-01.md` for exact institutional routes and numbered manual work; requests remain unsent.

1. Recover and visually inspect the **1926 Kwality Restaurant advertisement(s)** cited by the City inventory, recording date, title, page, address, proprietor, and exact trade name.
2. Review **1923–1926 Banner-Courier and Enterprise microfilm**, especially advertisements, licenses, business sales, relocations, pool/billiard notices, restaurant notices, and the days following the **30 April 1925** Main/Fifth–Sixth call (the former April 18 date was a transcription error).
3. Inspect the **1924 Oregon City directory** and any 1925–1927 Polk, telephone, or business directories for the address ladder at 501–511 Main and alphabetical entries for Farr, Kwality, Leland, Little, Timms, and Jager.
4. Inspect municipal restaurant, billiard/pool-hall, cigar, soft-drink, sign, and business-license ledgers and council/commission minutes for 1924–1927.
5. Retrieve the underlying 503 and Farr residence historic-resource research files, bibliographies, field notes, and newspaper citations.
6. Trace the provenance/date of the Farr's Pool Hall trade token and the New Kwality / Larry's Kwality artifacts; use them only as evidence of what they actually print.

## Current answer

- **503 in May 1925:** restaurant; business name and operator unresolved. **Kwality Restaurant by 1926** is reported by the official City synthesis, with the underlying advertisement still needed.
- **505 in May 1925:** billiards/cigars; business name and operator unresolved. **Farr's Pool Hall at 505** is independently confirmed at an undetermined date, but has not been bridged to May 1925.

## 3 September 2026 — Quality / Newton update

`E-179` / `S-141` now formally records the June 8 **427 Main, phone 517** advertisement and retains the original PDF. The August Newton reports (`E-180`) add **across the way from the laundry**; Newton-to-427 is probable only, and the 503 Kwality relationship remains unproved. See [1922-george-newton-427-main-address-test-2026-09-03](1922-george-newton-427-main-address-test-2026-09-03.md). This supplement does not expand the prior full-issue visual certification to other issues.
